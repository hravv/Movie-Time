    ...
    <header className='w-full flex flex-col md:flex-row justify-evenly md:py-12 shadow-2xl z-10 mt-8 md:mt-0'>
      ...
      <div className='titlesection w-full md:w-3/10 my-4 md:my-0'>
        <div className='title w-fit bg py-5 px-3 rounded-3xl mx-auto'>
          <h1 className='text-4xl lg:text-5xl text-amber-50 font-bold font-[Roboto_Slab] cursor-pointer' onClick={()=>setList('Home')} ><span className='text-neonblue'>M</span>ovie<span className='text-neonblue'>T</span>ime</h1>
        </div>
      </div>
      <div className='w-full md:w-4/10 flex items-center justify-center md:justify-normal xl:justify-end mt-4 md:mt-0'>
        <form onSubmit={handleSearchSubmit} className="flex items-center justify-end">
          <input
            type="text"
            name="search"
            className="search bg-white text-black h-7 md:h-8 border-none rounded-md pl-4 w-[8rem]  md:w-[12rem]"
            placeholder="Search"
            value={searchInput}
            onChange={(e) => setSearchInput(e.target.value)}
          />
          <button type="submit" className="mx-3 lg:mx-8 cursor-pointer w-fit" >
